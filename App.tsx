...
type Habit = { id: string; name: string; icon: string; target: number; color: string; createdAt: string };
...
const seedHabits: Habit[] = [
  { id: 'water', name: 'Drink water', icon: '💧', target: 4, color: '#79B9CD', createdAt: '2020-01-01' },
  { id: 'read', name: 'Read a little', icon: '📚', target: 1, color: '#E8B45C', createdAt: '2020-01-01' },
  { id: 'move', name: 'Move my body', icon: '🏃', target: 1, color: '#E89B81', createdAt: '2020-01-01' },
];
...
export default function App() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY).then((saved) => {
      if (saved) { const data = JSON.parse(saved); setHabits(data.habits || seedHabits); setLogs(data.logs || {}); }
  ...
  const updateCount = (habit: Habit, change: number) => {
    const key = todayKey();
    setLogs((old) => ({ ...old, [key]: { ...old[key], [habit.id]: Math.max(0, Math.min(habit.target, (old[key]?.[habit.id] || 0) + change)) } }));
  };
  ...
  return (
    <SafeAreaView style={styles.safe}><StatusBar style="dark" />
      <View style={styles.app}>
        ...
        <View style={styles.sectionRow}><Text style={styles.sectionTitle}>Today’s habits</Text><Pressable onPress={() => setEditing({ id: '', name: '', icon: '🌱', target: 1, color: COLORS[0], createdAt: todayKey() })}><Text style={styles.addText}>+ Add habit</Text></Pressable></View>

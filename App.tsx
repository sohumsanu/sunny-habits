...
function LegacyHabitEditor({ habit, onClose, onSave, onDelete }: { habit: Habit | null; onClose: () => void; onSave: (habit: Habit) => void; onDelete: (id: string) => void }) {
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalShade}>
      <View style={styles.sheet}>
        ...
        <View style={styles.detailTop}>
          ...
          <Text style={styles.detailTitle}>{draft.id ? 'Edit habit' : 'New habit'}</Text>
          <Pressable onPress={() => { if (draft.name.trim()) onSave({ ...draft, name: draft.name.trim(), icon: draft.icon.trim() || '✨', unit: draft.unit.trim() || 'steps' }); }}><Text style={styles.edit}>Save</Text></Pressable>
        </View>
        <ScrollView showsVerticalScrollIndicator={false}>
          <Text style={styles.fieldLabel}>NAME YOUR HABIT</Text>
          <TextInput autoFocus value={draft.name} onChangeText={(name) => setDraft({ ...draft, name })} placeholder="e.g. Drink water" placeholderTextColor="#B4A79B" style={styles.input}/>
          <Text style={styles.fieldLabel}>PICK ANY EMOJI</Text>
          <TextInput value={draft.icon} onChangeText={(icon) => setDraft({ ...draft, icon })} placeholder="✨" placeholderTextColor="#B4A79B" style={styles.input} maxLength={12} accessibilityLabel="Habit emoji"/>
          <Text style={styles.unitHint}>Use your phone’s emoji keyboard to choose any icon.</Text>
          <Text style={styles.fieldLabel}>DAILY GOAL</Text>
          <View style={styles.targetRow}>
            <Pressable onPress={() => setDraft({ ...draft, target: Math.max(1, draft.target - 1) })} style={styles.targetButton}><Text style={styles.stepText}>−</Text></Pressable>
            <View><Text style={styles.targetNumber}>{draft.target}</Text><Text style={styles.targetCaption}>{draft.target === 1 ? 'one check-off' : 'check-offs per day'}</Text></View>
            <Pressable onPress={() => setDraft({ ...draft, target: Math.min(20, draft.target + 1) })} style={[styles.targetButton, { backgroundColor: draft.color, borderColor: draft.color }]}><Text style={styles.stepPlus}>+</Text></Pressable>
          </View>
          <Text style={styles.fieldLabel}>WHAT ARE YOU COUNTING?</Text>
          <TextInput value={draft.unit} onChangeText={(unit) => setDraft({ ...draft, unit })} placeholder="e.g. glasses, pages, liters" placeholderTextColor="#B4A79B" style={styles.input} maxLength={24}/>
          <Text style={styles.unitHint}>Use a plural word for your count, like “glasses” or “pages”.</Text>
          {draft.id ? <Pressable onPress={() => Alert.alert('Delete habit?', `Remove ${draft.name}?`, [{ text: 'Keep it', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: () => onDelete(draft.id) }])}><Text style={styles.delete}>Delete habit</Text></Pressable> : null}
        </ScrollView>
      </View>
